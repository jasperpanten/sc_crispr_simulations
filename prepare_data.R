...
setwd("/cfs/klemming/home/p/panten/panten/projects/sam_simulations/sc_crispr_simulations/")
...
sce <- readRDS("../data/sce_gasperini_sam_finished.rds")

# remove count data since its not actually needed (only summary stats from deseq)

empty_here <- MatrixExtra::emptySparse(nrow(sce), ncol(sce), format = "R")
colnames(empty_here) <- colnames(sce)
rownames(empty_here) <- rownames(sce)

counts(sce) <- empty_here
logcounts(sce) <- empty_here

saveRDS(sce, "../data/sce_gasperini_sam_finished_empty.rds")
