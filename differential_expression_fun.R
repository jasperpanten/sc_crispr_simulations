...
filter_cells_per_pert <- function(sce, min_cells, pert_level, altExp = F) {
  ...
  if (altExp){
    ...
    # get perturbations with at least 'min_cells' cells
    cells_per_pert <- table(colData(sce)[ , pert_level ])
    perts_filter <- cells_per_pert >= min_cells
    
    # filter perturbation altExp based for perturbations passing 'min_cells' filter
    message("Removing ", sum(!perts_filter),
            " perturbations based on 'minimum cells per perturbation' filter.")
    sce[ , colData(sce)[ , pert_level] %in% names(which(perts_filter)) ]
...
test_differential_expression <- function(sce, pert_level, max_dist = NULL,
  ...
  output <- lapply(perts, FUN = test_de, sce = sce, pert_level = pert_level,
  ...
  method = as.character(substitute(de_function))
  if (method != "LFC") {
...
de_MAST <- function(pert_object, formula = ~pert, pvalue = c("hurdle", "cont", "disc"),
  ...
  sca <- SceToSingleCellAssay(pert_object, check_sanity = F)
  ...
  # JP: try and fix the part with one gene
  lfc <- tryCatch(
    withCallingHandlers({
      ...
      data.frame(primerid = genes, coef = as.numeric(zlm_fit@coefD)[2], ci.hi = NA_real_, ci.lo = NA_real_)
    })
...
de_SCEPTRE <- function(sim_object, formula = ~pert, 
                       pvalue = c("standard"),
                       parallel = FALSE) {
  ...
  grna_matrix_here <- as.matrix(rbind(abs((as.numeric(sim_object$pert) - 2)) * 100, (as.numeric(sim_object$pert) - 1) * 100))
  colnames(grna_matrix_here) <- colnames(sim_object)
  rownames(grna_matrix_here) <- c("notpert", "pert")
   
  grna_target_data_frame <- data.frame(grna_target = "pert", grna_id = "pert")
  ...
  discovery_pairs <- data.frame(
    grna_target = grna_target_data_frame$grna_target, 
    response_id = intersect(rowData(altExps(sim_object)[["cre_pert"]])[pert, ]$target_genes[[1]], rownames(sim_object))
  )
  sceptre_object %>%
...
annotate_dist_to_gene <- function(de_output, sce, pert_level) {
  ...
  # get perturbation coordinates from alternative experiment rowRanges
  pert_row_data <- rowRanges(altExp(sce, pert_level))
  pert_coords <- data.frame(pert_chr = seqnames(pert_row_data),
                            pert_start = start(pert_row_data),
                            pert_end = end(pert_row_data),
                            perturbation = pert_row_data$cre_target,
                            stringsAsFactors = FALSE)
